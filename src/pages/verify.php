...
<form method="POST" class="flex flex-col items-center justify-center h-screen gap-y-1" action="/api/verify">
    ...
    <?php endif; ?>
    <span class="text-xl font-bold">A verification code has been sent to your email, please enter it to complete registration!</span>
    <div class="flex gap-x-1">
        <?php foreach (range(0, 5) as $_): ?>
        <input name="digit[]" autocomplete="off" type="number" min="0" max="9" required class="w-12 p-1 text-center border-2 border-black rounded-md " />
        <?php endforeach; ?>
    </div>
